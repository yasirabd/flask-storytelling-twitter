...
class MainTagger:
    ...
    def __init__(self, fileLexicon, fileNGram, NGramType, maxAffixLength=3, Treshold=3, minWordFreq=0, modeAffixTree=0, debug=False, LambdaBigram=0.2, TwoPhaseType=0, beamFactor=500.0, useLexicon=0):
        ...
        self.SITE_ROOT = os.path.abspath(os.path.dirname(__file__))
        self.loadData()
    ...
    def loadData(self):
        try:
            ...
            if self.useLexicon != 0:
                inlex_url = os.path.join(self.SITE_ROOT, "..\..\data", "inlex.txt")
                cattable_url = os.path.join(self.SITE_ROOT, "..\..\data", "cattable.txt")
                self.dl = DicLexicon(inlex_url, cattable_url, self.model.getNumberTags())
